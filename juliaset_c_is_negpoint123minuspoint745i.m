...
fixpt1 = (1+sqrt(1.492+2.980i))/2; %These are the fixed points.
fixpt2 = (1-sqrt(1.492+2.980i))/2;

colormap([1 0 0; 1 1 1]); %Points numbered 1 (inside) will be colored red;
                           %Those numbered 2 (outside) will be white.
M = 2*ones(1001,1001);       %Initilize array of point colors to 2 (white).

for j=1:1001,               %Try initial values with imaginary parts between 
  y = -1 + (j-1)*.01;     % -.7 and .7
  for i=1:1001,             %and with real parts between
...
image([-12 -2],[-12 -2],M), %This plots the results.
pbaspect ([1 1 1]);
